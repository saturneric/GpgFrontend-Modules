...
// The token format, moved out of the Host's core unchanged: the wire is a
// contract between every copy of GpgFrontend, so not one byte of what Encode()
// writes or Decode() accepts may differ from what the core used to do. The
// fixtures in test/ were written by the core implementation and pin that.
...
auto EnsureSodiumInit() -> bool {
  static const bool kReady = sodium_init() >= 0;
  return kReady;
}

/// @p n bytes from libsodium's CSPRNG, or empty when it is unavailable.
auto RandomBytes(int n) -> QByteArray {
  if (!EnsureSodiumInit() || n < 0) return {};
...
auto DeriveBookSeed(const QString& phrase, BookSeed& seed) -> bool {
  ...
  if (rc != 0) {
    qWarning() << "argon2id derivation for instant-messaging book failed";
    return false;
  }
  return true;
}
...
auto BookBytesFor(const QString& phrase) -> QByteArray {
  static QMutex mutex;
  ...
  auto book = QByteArray(reinterpret_cast<const char*>(sub.data()),
...
auto FingerprintOfBook(const QByteArray& book) -> QString {
...
auto DeriveMaster(const QByteArray& book_bytes, const QByteArray& seed,
...
auto Encode(const QByteArray& pgp, const QString& phrase) -> QString {
  ...
  if (pgp.size() > kMaxPayloadBytes) {
    qWarning() << "instant-messaging payload too long:" << pgp.size()
               << "limit:" << kMaxPayloadBytes;
    return {};
  }

  const auto book = BookBytesFor(phrase.trimmed());

  const QByteArray seed = RandomBytes(kSeedLen);
  if (seed.size() != kSeedLen) return {};
  ...
  if (kSeedLen + m > kMaxWireBytes) {
    qWarning() << "instant-messaging frame exceeds the wire budget:" << m;
  ...
  inner.append(pgp);
  if (pad_len > 0) {
    const auto pad = RandomBytes(pad_len);
    if (pad.size() != pad_len) {
      sodium_memzero(master.data(), master.size());
      return {};
    }
...
auto Decode(const QString& text, const QString& phrase) -> DecodeResult {
  ...
  r.pgp_message = inner.mid(kHeaderLen, static_cast<int>(payload_len));
  r.ok = true;
  return r;
}

auto FormatVersion() -> int { return kVersion; }
...
auto GeneratePhrase() -> QString {
  ...
  while (out.size() < kPhraseLen) {
    const auto bytes = RandomBytes(static_cast<int>(kPhraseLen - out.size()));
    if (bytes.isEmpty()) {
      qWarning() << "csprng unavailable, cannot generate a book phrase";
      return {};
    }
