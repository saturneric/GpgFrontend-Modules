...
// Deprecated spellings, kept for one cycle so a module can be ported without
...
template <typename T>
[[deprecated("renamed to SdkMallocAsType: it is not a secure allocation")]]
auto SecureMallocAsType(std::size_t size) -> T* {
  return SdkMallocAsType<T>(size);
}

template <typename T>
[[deprecated("renamed to SdkReallocAsType: it is not a secure allocation")]]
auto SecureReallocAsType(T* ptr, std::size_t size) -> T* {
  return SdkReallocAsType<T>(ptr, size);
}
